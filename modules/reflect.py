...
class Reflect:
    ...
    def __init__(self, model, criterion=None, optimizer=None, device='cuda' if torch.cuda.is_available() else 'cpu'):
        ...
        if optimizer is None:
            self.optimizer = optim.Adam(
                self.model.critic.parameters(), lr=0.001)
        else:
    ...
    def train_critic(self, explanations, targets, num_epochs=1, batch_size=64):
        ...
        for epoch in range(num_epochs):
            ...
            print(
                f"Critic - Epoch {epoch+1}/{num_epochs}, Loss: {epoch_loss:.4f}, Acc: {epoch_acc:.2f}%")
    ...
    def get_feedback(self, explanations, targets):
        ...
        eval_loss, accuracy, _ = self.evaluate_explanations(
            explanations, targets)

        print(
            f"Critic Feedback - Loss: {eval_loss:.4f}, Accuracy: {accuracy:.2f}%")
