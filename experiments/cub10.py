"""
Experiment script for CUB-10 dataset.

This script runs the LSX experiment on the CUB-10 dataset.
"""
...
from data.datasets import get_cub10_dataloaders
import os
import torch
...
def run_experiment(config_path):
    """
    Run the CUB-10 experiment with the given configuration.
    ...
    base_save_dir = experiment_config['save_dir']
    ...
    print(f"Results will be saved in the experiment folder: {experiment_name}")
    ...
    fit_module = Fit(
        ...
        optimizer=optim.Adam(model.learner.parameters(),
                             lr=training_config['learning_rate_fit']),
        device=device
    )
    ...
    reflect_module = Reflect(
        model=model,
        criterion=nn.CrossEntropyLoss(),
        optimizer=optim.Adam(model.critic.parameters(),
                             lr=training_config['learning_rate_reflect']),
        device=device
    )
    ...
    revise_module = Revise(
        ...
        criterion=nn.CrossEntropyLoss(),
        optimizer=optim.Adam(model.learner.parameters(),
                             lr=training_config['learning_rate_revise']),
    ...
    vanilla_explanations, vanilla_inputs, vanilla_targets = explain_module.generate_explanations(
        critic_loader)
    ...
    vanilla_rr_acc = ridge_regression_accuracy(
        vanilla_explanations, vanilla_targets)
    vanilla_iies = inter_vs_intraclass_explanation_similarity(
        vanilla_explanations, vanilla_targets)
    ...
    if experiment_config['save_model']:
        torch.save(model.learner.state_dict(),
                   os.path.join(save_dir, "vanilla_model.pth"))
    ...
    for iteration in range(lsx_config['iterations']):
        ...
        explanations, inputs, targets = explain_module.generate_explanations(
            critic_loader)
        ...
        print(
            f"LSX model test accuracy after iteration {iteration+1}: {lsx_acc:.2f}%")
        ...
        plot_training_history(
            ...
            save_path=os.path.join(
                save_dir, f"lsx_revision_history_iter{iteration+1}.png")
        )

    # Final finetuning
    print("\n=== Final Finetuning ===")
    final_explanations, _, _ = explain_module.generate_explanations(
        train_loader)
    ...
    lsx_explanations, lsx_inputs, lsx_targets = explain_module.generate_explanations(
        critic_loader)
    ...
    lsx_iies = inter_vs_intraclass_explanation_similarity(
        lsx_explanations, lsx_targets)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run CUB-10 experiment")
    parser.add_argument(
        "--config", type=str, default="../configs/cub10_config.py", help="Path to configuration file")
